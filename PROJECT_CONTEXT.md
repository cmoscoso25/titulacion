...
- **Reportes:** `rep-view` zeroes padding. KPI grids: `auto-fit minmax` — nunca `repeat(N, 1fr)`.
  **Auditoría 2026-06-11:** KPIs institucionales (puntualidad, tiempos, hora peak) filtran `tipo="ESTUDIANTE"` en `RegistroIngreso`.
  `total_asistentes_b` = titulados + invitados → etiqueta "Total Presentes (Tit.+Inv.)" — nunca usar como base de %.
