...
## Sistema de permisos
Grupos Django: `ADMIN_TITULACION` / `ADMISION` / `CURRICULAR` / `ENTREGA_INVITACIONES` / `INGRESO` / `DACOM` / `SOLO_REPORTES`.
Ver `titulacion/permisos.py`. Funciones: `es_admin_titulacion()`, `es_admision()`, `es_curricular()`, `es_reportes()`, `es_entrega()`, `es_ingreso()`.
`acceso_reportes` permite ADMIN+CURRICULAR+SOLO_REPORTES — usado en las 5 rutas `/reportes/*`.
`perm_reportes` inyectado por context_processors en todos los templates.
QR: `titulacion/generador_qr.py` → `generar_qr_estudiante()`, `generar_qr_invitacion()`, `crear_imagen_qr()`.
Cambio contraseña: `/cambiar-contrasena/` (login_required) · link "Cambiar clave" en topbar de los 8 módulos.
